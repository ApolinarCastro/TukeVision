...
def _generate_physical_report(runtime, forensics):
    ...
    for cid in runtime._camera_ids:
        ...
        if tl:
            fc = tl.frame_sequence if tl.frame_sequence >= 0 else fc
        ...
        freeze = getattr(tl, "consecutive_identical", 0) if tl else 0
    ...
    try:
        ...
        orphan_readers = sum(1 for th in _th.enumerate() if "tukevision-rtsp-reader" in th.name and not th.is_alive())
    except Exception:
        pass
    report = {
        ...
        "cameras_available_start": len(runtime._camera_ids),
