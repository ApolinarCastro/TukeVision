...
* **DECISION**: ADAPT (Añadir `startup_grace_seconds` al `SourceManager`).
...
* **DECISION**: ADAPT (Añadir `consecutive_failure_count` antes de desencadenar recuperación).
...
* **DECISION**: ADAPT (Higiene de procesos: verificar muerte antes de reemplazar).
...
* **DECISION**: ADAPT (La recuperación exige confirmación de recepción física de frame `seq > old_seq`).
