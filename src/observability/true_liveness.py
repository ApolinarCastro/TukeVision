...
FREEZE_HASH_WINDOW = 5  # diagnostic only: static scenes may have identical pixels
...
class TrueLivenessTracker:
    ...
    def observe_frame(self, camera_id: str, frame, frame_index: int, capture_state: str = "OPEN", reader_tid: Optional[int] = None, *, acquired_at=None, generation=None):
        now = time.monotonic() if acquired_at is None else float(acquired_at)
        h = _frame_hash(frame) if frame is not None else None
        with self._lock:
            self._capture_state[camera_id] = capture_state
            if generation is not None and generation != self._generation.get(camera_id):
                self._generation[camera_id] = generation
                self._seq[camera_id] = -1
            prev_seq = self._seq.get(camera_id, -1)
            if frame is None or frame_index <= prev_seq:
                return  # replay/repoll must not rejuvenate the acquisition timestamp
    ...
    def _state_for(self, camera_id: str) -> tuple[str, bool, bool]:
        ...
        if age <= self._threshold and not reader_stale and capture_state in ("OPEN", "READING"):
